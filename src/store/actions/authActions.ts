...
export const signup =
  (name: string, treasury_name: string, email: string, password: string) =>
  async (dispatch: any) => {
    try {
      dispatch({
        type: SET_AUTH_LOADING,
...
export const updateProfile =
  (name: string, about: string) => async (dispatch: any) => {
    try {
      dispatch({
        type: SET_AUTH_LOADING,
        payload: true,
      ...
      console.log('...Error in updateProfile authActions', error);
      Alert.alert(getErrorMessage(error));
    } finally {
      dispatch({
        type: SET_AUTH_LOADING,
